...
;( function() {
  ...
  Tree.prototype = {
    ...
    positionNodes: function() {
      ...
      for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
        ...
        if (node.id !== 0 && !(node.parent().id === 0 && this.CONFIG.hideRootNode) && !node.hideConnectorToParent) {
          console.log("setConnectionToParent " + node.hideConnectorToParent);
          this.setConnectionToParent(node, hidePoint); // skip the root node
        }
        if (node.pseudo) {
          this.connectMarriage(node);
        }
     ...
     * @returns {Tree}
     */
    connectMarriage: function( treeNode ) {
      var leftSibling = treeNode.leftSibling(),
        rightSibling = treeNode.rightSibling(),
        leftX = leftSibling.X + leftSibling.width,
        rightX = leftX + this.CONFIG.siblingSeparation,
        y = treeNode.Y + treeNode.height/2,
        sp = leftX + ',' + y,
        ep = rightX + ',' + y,
        pathString = ["M", sp, 'L', ep].join(" ");
    ...
    getPathString: function( from_node, to_node, stacked ) {
      ...
      for (var i = 0; i < to_node.parent().childrenCount(); i++) {
        var h = to_node.parent().childAt(i).height;
        if (h > maxParentHeight) maxParentHeight = h;
      }

      if ( orientation == 'NORTH' || orientation == 'SOUTH' ) {
        P1.y = P2.y = (startPoint.y + endPoint.y + maxParentHeight/2) / 2;
  ...
  TreeNode.prototype = {
    ...
    reset: function( nodeStructure, id, parentId, tree, stackParentId ) {
      ...
      this.hideConnectorToParent = nodeStructure.hideConnectorToParent === false ? false : ( nodeStructure.hideConnectorToParent || tree.CONFIG.node.hideConnectorToParent );
    ...
    connectorPoint: function(startPoint) {
      ...
      if ( orient == 'NORTH' ) {
        ...
        point.y = (this.pseudo) ? this.Y + this.height/2 : (startPoint) ? this.Y + this.height : this.Y;
      }
      else if (orient == 'SOUTH') {
        point.x = (this.pseudo) ? this.X - this.Tree().CONFIG.subTeeSeparation/2 : this.X + this.width/2;
        point.y = (this.pseudo) ? this.Y + this.height/2 : (startPoint) ? this.Y : this.Y + this.height;
      }
      else if (orient == 'EAST') {
        point.x = (this.pseudo) ? this.X + this.width/2 : (startPoint) ? this.X : this.X + this.width;
        point.y = (this.pseudo) ? this.Y - this.Tree().CONFIG.subTeeSeparation/2 : this.Y + this.height/2;
      }
      else if (orient == 'WEST') {
        point.x = (this.pseudo) ? this.X + this.width/2 : (startPoint) ? this.X + this.width : this.X;
        point.y = (this.pseudo) ? this.Y - this.Tree().CONFIG.subTeeSeparation/2 : this.Y + this.height/2;
      }
      return point;
    },
